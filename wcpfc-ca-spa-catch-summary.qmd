...
@tbl-ccm-spa-catch contains the same recent average catch shown in @fig-spa-catch-gear and @fig-spa-catch-ccm for each fishery subject to MP control. It also contains maximum allowable cuts (5 %) and increases (10 %) each time the MP is applied based on the constraints in the proposed CMM[^2].
